...
class VF_Events_Thank_You {
  ...
  const MIGRATION_VERSION = '20260709_1';
  ...
  function __construct($file) {
    ...
    add_action(
      ...
      array($this, 'thank_you_posts_custom_column'),
      10, 2
    );
    add_filter('post_class', array($this, 'thank_you_post_class'), 10, 3);
    add_action('admin_head-edit.php', array($this, 'highlight_past_thank_you_pages'));
    add_action(
      'manage_posts_extra_tablenav',
      array($this, 'thank_you_posts_extra_tablenav')
    );
    add_action(
      'admin_post_vf_events_delete_past_thank_you_pages',
      array($this, 'admin_post_delete_past_thank_you_pages')
    );
    add_action(
    ...
    add_action('init', array(__CLASS__, 'clear_cleanup_schedule'), 30);
    add_action('trashed_post', array($this, 'sync_event_toggle_after_delete'));
    add_action('before_delete_post', array($this, 'sync_event_toggle_after_delete'));
    add_action('untrashed_post', array($this, 'sync_event_toggle_after_restore'));
  }
  ...
  public function admin_post_delete_thank_you_page() {
    ...
    if ( ! $thank_you_id || get_post_type($thank_you_id) !== self::POST_TYPE) {
      $this->redirect_to_settings('delete_failed');
    }

    $event_id = self::get_parent_event_id($thank_you_id);

    $deleted = wp_trash_post($thank_you_id);
  ...
  public function admin_notices() {
    ...
    if (
      ...
      isset($_GET['vf_events_deleted_past_thank_you_pages'])
    ) {
      $deleted = max(
        0,
        intval($_GET['vf_events_deleted_past_thank_you_pages'])
      );
      ...
      printf(
        ...
        esc_html(sprintf(
          _n(
            'Deleted %d past thank you page.',
            'Deleted %d past thank you pages.',
            $deleted,
            'vfwp'
          ),
          $deleted
  ...
  static public function build_for_event($event_id) {
    ...
    $postarr = array(
      ...
      'post_name' => self::get_event_code($event_id),
  ...
  static public function delete_for_event($event_id) {
    ...
    return (bool) wp_trash_post($thank_you_id);
  }
  ...
  static private function get_event_ids_with_thank_you_enabled() {
    ...
    return array_values(array_filter(array_map('intval', $event_ids), function($event_id) {
      return self::event_includes_thank_you_page($event_id);
    }));
  }
  ...
  static public function get_event_thank_you_url($event_id) {
    ...
    return home_url('/thank-you/' . self::get_event_code($event_id) . '/');
  }

  static private function get_event_code($event_id) {
    $code = get_post_field('post_name', $event_id);

    if ( ! is_string($code) || $code === '') {
      $code = 'event-' . (int) $event_id;
    }
  ...
  public function migrate_legacy_thank_you_pages() {
    if (
      ! self::is_enabled() ||
      get_option('vf_events_thank_you_migration_version') === self::MIGRATION_VERSION
    ) {
      return;
    }
    ...
    foreach ($legacy_ids as $legacy_id) {
      ...
      $event_id = (int) get_post_meta(
        $legacy_id,
        self::META_IS_THANK_YOU_PAGE,
        true
      );

      if (
        ! $legacy ||
        $legacy->post_type !== VF_Events::type() ||
        $legacy->post_name !== 'thank-you' ||
        (int) $legacy->post_parent !== $event_id ||
        ! $event_id ||
        get_post_type($event_id) !== VF_Events::type()
      ) {
        continue;
      }
    ...
    foreach (self::get_event_ids_with_thank_you_enabled() as $event_id) {
      if (
        get_post_status($event_id) === 'publish' &&
        ! self::get_thank_you_page_id($event_id) &&
        ! self::build_for_event($event_id)
      ) {
        $migration_complete = false;
      }
    }
    ...
    if ($migration_complete) {
      update_option(
        'vf_events_thank_you_migration_version',
  ...
  static public function is_event_past($event_id) {
  ...
  public function sync_event_toggle_after_delete($post_id) {
    if (get_post_type($post_id) !== self::POST_TYPE) {
      return;
    }

    $event_id = self::get_parent_event_id($post_id);
  ...
  public function sync_event_toggle_after_restore($post_id) {
    if (get_post_type($post_id) !== self::POST_TYPE) {
      return;
    }
  ...
  public function thank_you_posts_columns($columns) {
    unset($columns['date']);
    $columns['vf_thank_you_event_code'] = __('Event code', 'vfwp');
    $columns['vf_thank_you_parent_event'] = __('Parent event', 'vfwp');
    $columns['vf_thank_you_event_date'] = __('Event date', 'vfwp');
    $columns['vf_thank_you_status'] = __('Event status', 'vfwp');
    $columns['date'] = __('Published', 'vfwp');

    return $columns;
  }

  public function thank_you_posts_custom_column($column, $post_id) {
    $event_id = self::get_parent_event_id($post_id);

    if ($column === 'vf_thank_you_event_code') {
      echo esc_html($event_id ? self::get_event_code($event_id) : '');
      return;
    }

    if ($column === 'vf_thank_you_parent_event') {
      if ( ! $event_id || get_post_type($event_id) !== VF_Events::type()) {
        echo '&mdash;';
        return;
      }
      ...
      printf(
        ...
        esc_html__('View event', 'vfwp')
      );
      return;
    }

    if ($column === 'vf_thank_you_event_date') {
      $start = $event_id ? self::get_event_start_timestamp($event_id) : 0;
      $end = $event_id ? self::get_event_end_timestamp($event_id) : 0;
      echo esc_html(self::format_event_date_range($start, $end));
      return;
    }

    if ($column === 'vf_thank_you_status') {
      echo esc_html(
        ! $event_id
          ? __('Missing event', 'vfwp')
          : (
            self::is_event_past($event_id)
              ? __('Past event', 'vfwp')
              : __('Upcoming event', 'vfwp')
          )
      );
    }
  }

  public function thank_you_post_class($classes, $class, $post_id) {
    if (
      is_admin() &&
      get_post_type($post_id) === self::POST_TYPE &&
      self::is_event_past(self::get_parent_event_id($post_id))
    ) {
      $classes[] = 'vf-events-thank-you--past';
    }

    return $classes;
  }

  public function highlight_past_thank_you_pages() {
    $screen = get_current_screen();
    ...
    $past_ids = array_values(array_filter(
      self::get_existing_thank_you_page_ids(),
      function($post_id) {
        return self::is_event_past(self::get_parent_event_id($post_id));
      }
    ));
    ...
    foreach ($past_ids as $post_id) {
      $selector = '#post-' . (int) $post_id;
      $cells[] = $selector . '>th';
      $cells[] = $selector . '>td';
      $hover_cells[] = $selector . ':hover>th';
      $hover_cells[] = $selector . ':hover>td';
    }

    echo '<style>';
    echo implode(',', $cells) . '{background:#fcdddd!important}';
    echo implode(',', $hover_cells) . '{background:#f8caca!important}';
    echo '</style>';
  }

  public function thank_you_posts_extra_tablenav($which) {
    $screen = get_current_screen();
    ...
    $past_ids = self::get_past_thank_you_page_ids();
    ...
    $url = wp_nonce_url(
      admin_url('admin-post.php?action=vf_events_delete_past_thank_you_pages'),
      'vf_events_delete_past_thank_you_pages'
    );
    ?>
    <div class="alignleft actions vf-events-thank-you-past-actions">
      <a
        href="<?php echo esc_url($url); ?>"
        class="button vf-events-button--danger"
        onclick="return window.confirm('<?php echo esc_js(__('Delete all thank you pages for past events?', 'vfwp')); ?>');"
      ><?php esc_html_e('Delete all past thank you pages', 'vfwp'); ?></a>
    </div>
    <?php
  }

  public function admin_post_delete_past_thank_you_pages() {
    if ( ! current_user_can('manage_options')) {
      wp_die(esc_html__('You are not allowed to delete thank you pages.', 'vfwp'));
    }

    check_admin_referer('vf_events_delete_past_thank_you_pages');

    $deleted = 0;

    foreach (self::get_past_thank_you_page_ids() as $thank_you_id) {
      if (
        get_post_type($thank_you_id) === self::POST_TYPE &&
        wp_trash_post($thank_you_id)
      ) {
        $deleted++;
      }
    }
    ...
    wp_safe_redirect(add_query_arg(array(
      'post_type' => self::POST_TYPE,
      'vf_events_deleted_past_thank_you_pages' => $deleted,
    ), admin_url('edit.php')));
    exit;
  }

  static private function get_past_thank_you_page_ids() {
    return array_values(array_filter(
      self::get_existing_thank_you_page_ids(),
      function($post_id) {
        $event_id = self::get_parent_event_id($post_id);

        return $event_id && self::is_event_past($event_id);
      }
    ));
  }

  static private function format_event_date_range($start, $end) {
    if ( ! $start) {
      return '';
    }

    if ( ! $end || wp_date('Y-m-d', $start) === wp_date('Y-m-d', $end)) {
      return wp_date('j F Y', $start);
    }

    if (wp_date('F Y', $start) === wp_date('F Y', $end)) {
      return wp_date('j', $start) . '-' . wp_date('j F Y', $end);
    }

    if (wp_date('Y', $start) === wp_date('Y', $end)) {
      return wp_date('j F', $start) . ' - ' . wp_date('j F Y', $end);
    }

    return wp_date('j F Y', $start) . ' - ' . wp_date('j F Y', $end);
  }

  public function sync_thank_you_post_to_event($post_id, $post, $update) {
    static $syncing = false;

    if ($syncing || wp_is_post_revision($post_id) || wp_is_post_autosave($post_id)) {
      return;
    }
    ...
    $event_code = self::get_event_code($event_id);

    if ($post->post_title === $event_title && $post->post_name === $event_code) {
      return;
    }
