...
<article class="vf-summary vf-summary--news allItems vf-u-margin__bottom--1200" data-jplist-item>
  <time class="vf-summary__date vf-u-text-color--grey" style="margin-left: 0;" data-eventtime="<?php the_time('Ymd'); ?>"
  ...
  <div class="vf-u-display-none">
    ...
    <span class="unit <?php 
        $uni_list = [];
        foreach( $unit as $uni ) {
          $uniSlug = 'unit-' . $uni->slug; 
          $uni_list[] = strtolower(str_replace(' ', '-', $uniSlug)); }
    ...
    <span class="site <?php 
        $loc_list = [];
        foreach( $site as $loc ) { 
          $locSlug = 'site-' . $loc->slug; 
          $loc_list[] = strtolower(str_replace(' ', '-', $locSlug)); }
