...
$(function() {
  ...
  $("#search-button-submit").click(function(event){
  ...
  function submitSearch() {
   ...
   // }, 1500); // Will do the ajax stuff after xxx ms
  }

  var customPreRenderFunction = function(info) {
    // console.log('here',info);
    var results = '';
    var results = '<p class="vf-text-body vf-text-body--3 | vf-u-text-color--ui--black | vf-u-margin__bottom--800">' + info['info']['page']['total_result_count'] + ' results found for "' + info['info']['page']['query'] +'"';
    if (activeFacet.what) {
  ...
  var customRenderFunction = function(document_type, item) {
    ...
    if (item['highlight']['body']) {
      var summary = item['highlight']['body'].substring(0, 150);
    } else {
      var summary = item['body'].substring(0, 150);
    }

    var description =  '<p class="vf-summary__meta" style="margin-bottom: 8px;">' + item['sections'][0] + '</p>';
    if (item['what'] == 'People directory') {
      var description = '<p class="vf-summary__meta" style="margin-bottom: 8px;">' + item['people-description'] + '</p>';
    }
    else if (item['what'] == 'News') {
      var description = '<p class="vf-summary__meta" style="margin-bottom: 8px;">' + item['news-description'] + '</p>';
    }
    else if (item['page-description']) {
      var description = '<p class="vf-summary__meta" style="margin-bottom: 8px;">' + item['page-description'] + '</p>';
    }


    var out = '<article class="vf-summary">' +
      // '<span class="vf-summary__date">' + item['published_at'] + '</span>' +
      '<h3 class="vf-summary__title" style="margin-bottom: 4px;">' +
  ...
  facetSelect.addEventListener('click', (event) => {
    if (event.target.getAttribute("data-id") == 'all') {
      activeFacet = '{}';
    } else {
      activeFacet = {'what': event.target.getAttribute("data-id")};
    }
  ...
  function handleVisibility() {
    ...
    else if (activeFacet.what == "Alumni") {
