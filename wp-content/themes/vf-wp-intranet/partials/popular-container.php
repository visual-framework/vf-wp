<section class="vf-summary-container" style="margin-top: 2.5rem !important;">
  <div class="vf-section-header">
    <p class="vf-section-header__heading">Quick links </p>
  </div>

  <div class="vf-section-content | vf-u-margin__top--200">
    <div class="vf-tabs">
      <ul class="vf-tabs__list | vf-u-margin__top--0" data-vf-js-tabs>
        <li class="vf-tabs__item">
          <a class="vf-tabs__link" href="#vf-tabs__section--1"
            data-vf-js-location-nearest-activation-target="barcelona">Barcelona</a>
        </li>
        <li class="vf-tabs__item">
          <a class="vf-tabs__link" href="#vf-tabs__section--2"
            data-vf-js-location-nearest-activation-target="grenoble">Grenoble</a>
        </li>
        <li class="vf-tabs__item">
          <a class="vf-tabs__link" href="#vf-tabs__section--3"
            data-vf-js-location-nearest-activation-target="hamburg">Hamburg</a>
        </li>
        <li class="vf-tabs__item">
          <a class="vf-tabs__link" href="#vf-tabs__section--4"
            data-vf-js-location-nearest-activation-target="default">Heidelberg</a>
        </li>
        <li class="vf-tabs__item">
          <a class="vf-tabs__link" href="#vf-tabs__section--5"
            data-vf-js-location-nearest-activation-target="rome">Rome</a>
        </li>
      </ul>
    </div>
    <div class="vf-tabs-content" data-vf-js-tabs-content="">

      <!-- Barcelona -->
      <section class="vf-tabs__section" id="vf-tabs__section--1">
        <div class="vf-grid vf-grid__col-5">
          <!-- 1 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/administration/administration-in-barcelona/">
                  Administration
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/coronavirus/">
                  COVID-19 Information
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/communications/design-resources/">
                  Design resources
                </a>
              </li>
            </ul>
          </div>
          <!-- 2 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="http://gcal.embl.de/">
                  EMBL Calendar
                </a>
              </li>

              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/embl-programme/">
                  EMBL Programme
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/human-resources/health-insurance/">
                  Health insurance
                </a>
              </li>
            </ul>
          </div>
          <!-- 3 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://ppms.embl.de/start/">
                  PPMS
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://saperp.embl.de/nwbc">
                  SAP
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://itsupport.embl.de/index.php?/Tickets/Submit">
                  Service request
                </a>
              </li>
            </ul>
          </div>
          <!-- 4 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://staffassociation.embl.org/">
                  Staff Association
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/it-services/network-access-barcelona/">
                  VPN
                </a>
              </li>
            </ul>
          </div>
          <div>
          </div>
        </div>
      </section>

      <!-- Grenoble -->

      <section class="vf-tabs__section" id="vf-tabs__section--2">
        <div class="vf-grid vf-grid__col-5">
          <!-- 1 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/administration/administration-in-grenoble/">
                  Administration
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/coronavirus/">
                  COVID-19 Information
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/communications/design-resources/">
                  Design resources
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="http://gcal.embl.de/">
                  EMBL Calendar
                </a>
              </li>
            </ul>
          </div>
          <!-- 2 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/embl-programme/">
                  EMBL Programme
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/human-resources/health-insurance/">
                  Health insurance
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/scientific-services-and-facilities/grenoble-library/">
                  Library
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://ppms.embl.de/start/">
                  PPMS
                </a>
              </li>
            </ul>
          </div>
          <!-- 3 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://booking.embl.fr/Web/view-schedule.php">
                  Room reservation
                </a>
              </li>

              <li class="vf-list__item">
                <a class="vf-list__link" href="https://saperp.embl.de/nwbc">
                  SAP
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://itsupport.embl.de/index.php?/Tickets/Submit">
                  Service request
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://staffassociation.embl.org/">
                  Staff Association
                </a>
              </li>
            </ul>
          </div>
          <!-- 4 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/it-services/">
                  IT Services
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://wiki.embl.fr">
                  Wiki
                </a>
              </li>
            </ul>
          </div>
          <div>
          </div>
        </div>
      </section>

      <!-- Hamburg -->

      <section class="vf-tabs__section" id="vf-tabs__section--3">
        <div class="vf-grid vf-grid__col-5">
          <!-- 1 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/administration/administration-in-hamburg/">
                  Administration
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/coronavirus/">
                  COVID-19 Information
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/communications/design-resources/">
                  Design resources
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="http://gcal.embl.de/">
                  EMBL Calendar
                </a>
              </li>
            </ul>
          </div>
          <!-- 2 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/embl-programme/">
                  EMBL Programme
                </a>
              </li>

              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/human-resources/health-insurance/">
                  Health insurance
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/scientific-services-and-facilities/szilard-library/">
                  Library
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/research/orcid/">
                  Orcid
                </a>
              </li>
            </ul>
          </div>
          <!-- 3 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://ppms.embl.de/start/">
                  PPMS
                </a>
              </li>

              <li class="vf-list__item">
                <a class="vf-list__link" href="https://rooms.embl.de/HH-OFFICES">
                  Room reservation
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://saperp.embl.de/nwbc">
                  SAP
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://itsupport.embl.de/index.php?/Tickets/Submit">
                  Service request
                </a>
              </li>
            </ul>
          </div>
          <!-- 4 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://staffassociation.embl.org/">
                  Staff Association
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/it-services/network-access-hamburg/">
                  VPN
                </a>
              </li>
            </ul>
          </div>
          <div>
          </div>
        </div>
      </section>

      <!-- Heidelberg -->

      <section class="vf-tabs__section" id="vf-tabs__section--4">
        <div class="vf-grid vf-grid__col-5">
          <!-- 1 -->
          <div>
            <ul class="vf-list">
            <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/sites/heidelberg/accommodation/">
                  Accommodation
                </a>
              </li>

              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/coronavirus/">
                  COVID-19 Information
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/communications/design-resources/">
                  Design resources
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="http://gcal.embl.de/">
                  EMBL Calendar
                </a>
              </li>
            </ul>
          </div>
          <!-- 2 -->
          <div>
            <ul class="vf-list">
            <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/embl-programme/">
                  EMBL Programme
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/human-resources/health-insurance/">
                  Health insurance
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/scientific-services-and-facilities/szilard-library/">
                  Library
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/about/info/food-services-heidelberg/">
                  Lunch menu
                </a>
              </li>
            </ul>
          </div>
          <!-- 3 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/photolab/">
                  Photolab
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://ppms.embl.de/start/">
                  PPMS
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://rooms.embl.de">
                  Room reservation
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://saperp.embl.de/nwbc">
                  SAP
                </a>
              </li>
            </ul>
          </div>
          <!-- 4 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://itsupport.embl.de/index.php?/Tickets/Submit">
                  Service request
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/documents/embl-shuttle-timetable/">
                  EMBL shuttle timetable
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://staffassociation.embl.org/">
                  Staff Association
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/it-services/network-access-heidelberg/">
                  VPN
                </a>
              </li>
            </ul>
          </div>
          <div>
          </div>
        </div>
      </section>

      <!-- Rome -->

      <section class="vf-tabs__section" id="vf-tabs__section--5">
        <div class="vf-grid vf-grid__col-5">
          <!-- 1 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/administration/administration-in-rome/">
                  Administration
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://www.embl.org/internal-information/coronavirus/">
                  COVID-19 Information
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/communications/design-resources/">
                  Design resources
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="http://gcal.embl.de/">
                  EMBL Calendar
                </a>
              </li>
            </ul>
          </div>
          <!-- 2 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/human-resources/health-insurance/">
                  Health insurance
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/scientific-services-and-facilities/szilard-library/">
                  Library
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://oc.embl.de/">
                  Owncloud
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://ppms.embl.de/start/">
                  PPMS
                </a>
              </li>

            </ul>
          </div>
          <!-- 3 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://rooms.embl.de/MR-ROOM">
                  Room reservation
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://saperp.embl.de/nwbc">
                  SAP
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link"
                  href="https://www.embl.org/internal-information/it-services/network-access-heidelberg/">
                  VPN
                </a>
              </li>
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://itsupport.embl.de/index.php?/Tickets/Submit">
                  Service request
                </a>
              </li>
            </ul>
          </div>
          <!-- 4 -->
          <div>
            <ul class="vf-list">
              <li class="vf-list__item">
                <a class="vf-list__link" href="https://staffassociation.embl.org/">
                  Staff Association
                </a>
              </li>
            </ul>
          </div>
          <div>
          </div>
        </div>
      </section>

    </div>
  </div>
</section>
