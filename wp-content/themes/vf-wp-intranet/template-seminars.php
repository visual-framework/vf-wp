      ...
      <div class="embl-content-hub-loader | vf-grid vf-grid__col-1" data-jplist-group="data-group-1">
        ...
        if( ! empty( $data ) ) {
            foreach( $data as $event ) {
              ...
              //   $address = '';
              // }
              $partner_class = ($type === 'Partner Events') ? ' partner-event' : '';

              echo '<article class="vf-summary vf-summary--event vf-u-margin__bottom--0 vf-u-padding__bottom--800 ' . $partner_class . ' | customDivider" data-jplist-item>';
                ...
                <a href="http://www.google.com/calendar/render?action=TEMPLATE&text=' . $event->title . '&ctz=Europe/Berlin&location=' . $event->field_event_venue . '&dates=' . $calendarStartDate . 'T' . $calendarStartTime . '00/' . $calendarStartDate . 'T' . $calendarEndTime . '00'.'&sprop=name:" target="_blank" rel="nofollow">Add to calendar</a>
               </span></p>';
    ...
    <div>
        <div class="vf-u-background-color-ui--off-white">
          <div class="vf-content vf-u-padding--400">
            ...
            href="https://www.embl.org/internal-information/seminars/how-to/">About this section</a></p>
...
document.addEventListener("DOMContentLoaded", function() {
  // Location checkboxes
  const grenobleCheckbox = document.getElementById("location-4");
  const romeCheckbox = document.getElementById("location-6");
  const virtualCheckbox = document.getElementById("location-2");

  // Partner Events type checkbox
  const partnerTypeCheckbox = document.getElementById("type-18");

  // All partner events
  const partnerEvents = document.querySelectorAll(".partner-event");

  // Hide all partner events by default
  partnerEvents.forEach(event => event.style.display = "none");

  function updatePartnerEvents() {
    partnerEvents.forEach(event => {
      const locationElement = event.querySelector(".vf-summary__location");
      const locationText = locationElement ? locationElement.textContent.toLowerCase() : "";

      const isVirtual = locationText.includes("virtual");

      let show = false;

      // Show if the Partner Events type checkbox is checked
      if (partnerTypeCheckbox.checked) {
        show = true;
      } else {
        // Show based on location checkboxes
        if (locationText.includes("embl grenoble") && grenobleCheckbox.checked) {
          if (!isVirtual || (isVirtual && virtualCheckbox.checked)) show = true;
        }
        if (locationText.includes("embl rome") && romeCheckbox.checked) {
          if (!isVirtual || (isVirtual && virtualCheckbox.checked)) show = true;
        }
      }
  ...
  [grenobleCheckbox, romeCheckbox, virtualCheckbox, partnerTypeCheckbox].forEach(checkbox => {
    checkbox.addEventListener("change", updatePartnerEvents);
  });
});
 </script> 
