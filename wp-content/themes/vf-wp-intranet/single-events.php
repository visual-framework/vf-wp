...
$endCal = $end_date ? DateTime::createFromFormat('j M Y', $end_date) : null;
...
if (!empty($end_date)) {if (empty($end_time)) {
  // No end time set: assume all-day event, add 1 day to end date
  $endCal->modify('+1 day');
}
  $calendar_end_date = '/' . $endCal->format('Ymd');
}
else {
...
<div class="vf-grid vf-grid__col-3 | vf-u-grid-gap--800 | vf-content">
  ...
  <div>
    ...
    <p class="vf-text-body vf-text-body--3"><span style="font-weight: 600;">Date:</span>
     ...
     <a href="http://www.google.com/calendar/render?action=TEMPLATE&text=<?php the_title(); ?>&ctz=Europe/Berlin&dates=<?php echo $start->format('Ymd') . $calendar_start_time; ?><?php echo $calendar_end_date . $calendar_end_time; ?>&sprop=name:" target="_blank" rel="nofollow">Add to calendar</a>
    </span>
