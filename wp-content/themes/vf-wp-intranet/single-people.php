...
<section class="embl-grid embl-grid--has-centered-content">
    ...
    <article class="vf-profile vf-profile--medium vf-profile--inline">
      ...
      <p class="vf-profile__uuid">
        ...
        <a class="vf-profile__link vf-profile__link--secondary"
          href="https://europepmc.org/authors/<?php echo esc_attr($orcid); ?>"><?php echo esc_html($orcid); ?></a>
      </p>
      <?php } ?>
    </article>
  </div>
  <div>
  <?php
      if ($outstation != 'EMBL-EBI') { ?>
      <p class="vf-text-body vf-text-body--5 vf-u-margin__top--400"><a href="https://privacysettings.embl.de/" target="_blank">[Privacy settings]</a></p>
      <?php } ?>
  </div>
</section>
...
<script>
  ...
  function emblOntologyFindTeamByNameInsensitive(name) {
    if (typeof emblTaxonomy === "undefined" || !emblTaxonomy.terms) {
      return false;
    }

    // Normalize terms to array
    const terms = Object.values(emblTaxonomy.terms);
    const lowerName = name.trim().toLowerCase();

    // Exact match (case-insensitive)
    for (const term of terms) {
      if (term.name && term.name.toLowerCase() === lowerName) {
        return term;
      }
    }

    // Partial match (handles e.g. "Zaugg Group" vs "Zaugg")
    for (const term of terms) {
      if (term.name && term.name.toLowerCase().includes(lowerName)) {
        return term;
      }
      if (term.name && lowerName.includes(term.name.toLowerCase())) {
        return term;
      }
    }
  ...
  // Assign URLs to team links
  function emblPeoplePagesGroupLinkAssign() {
    const links = document.querySelectorAll("[data-embl-js-group-link]");

    if (!links.length) {
      console.warn("No `[data-embl-js-group-link]` elements found");
      return;
    }

    for (const element of links) {
      const team = emblOntologyFindTeamByNameInsensitive(element.dataset.emblJsGroupLink);

      if (team && team.url) {
        element.href = team.url; // only overwrite if valid URL
      } else {
        console.warn("No ontology match, keeping fallback:", element.dataset.emblJsGroupLink);
      }
    }
  }

  // Wait for emblTaxonomy to load before assigning links
  function waitForEmblTaxonomy(callback, interval = 200, timeout = 5000) {
    ...
    (function check() {
      if (typeof emblTaxonomy !== "undefined" && emblTaxonomy.terms) {
        ...
        console.warn("emblTaxonomy never loaded within timeout");
      }
    })();
  }

  // Run assignment once taxonomy is ready
  waitForEmblTaxonomy(() => {
