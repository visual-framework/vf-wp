...
// Run the async version instead of the old function
add_action( 'vfwp_intranet_cron_process', 'vfwp_intranet_cron_process_people_data_async' );
...
function vfwp_intranet_cron_process_people_data_async() {
    ...
    $api_url = 'https://content.embl.org/static-api/all-staff-list-intranet.json';
    ...
    foreach ($data['rows'] as $person) {
        ...
        /*
        |--------------------------------------------------------------------------
        | Normalize Positions (Primary First, Max 4)
        |--------------------------------------------------------------------------
        */
        ...
        if (!empty($positions) && is_array($positions)) {
            ...
            // Limit to 4
            $positions = array_slice($positions, 0, 4);

            foreach ($positions as $index => $position) {
                $normalized_positions[$index + 1] = array(
                    'positions_name' => $position['job_title'] ?? '',
        ...
        */
        ...
        // Add dynamic positions (1–4)
        for ($i = 1; $i <= 4; $i++) {
            $fields_to_check["positions_name_$i"] = $normalized_positions[$i]['positions_name'] ?? '';
            $fields_to_check["team_name_$i"]      = $normalized_positions[$i]['team_name'] ?? '';
            $fields_to_check["is_primary_$i"]     = $normalized_positions[$i]['is_primary'] ?? '';
        }
