...
function vfwp_intranet_cron_process_bioit_data() {
  ...
  $raw_data = json_decode($xmldata, true);
  if (!empty($raw_data) && is_array($raw_data)) {
    insert_bioit_posts_from_json($bioit_json_feed_api_endpoint);
  }
  else {
...
function insert_bioit_posts_from_json($bioit_json_feed_api_endpoint) {
  if ( ! is_admin() ) {
     require_once( ABSPATH . 'wp-admin/includes/post.php' );
  }

  $raw_content = file_get_contents($bioit_json_feed_api_endpoint);
  $raw_content_decoded = json_decode($raw_content, true);
  $bioit_data = $raw_content_decoded;

  
    if (!empty($bioit_data) && is_array($bioit_data)) {
        foreach ($bioit_data as $key => $person) {
        $title = $person['channel']['item']['title'];
        $url = $person['channel']['item']['link'];
