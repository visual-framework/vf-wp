    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            ...
            $post_data = array(
                ...
                'excerpt'           => get_the_excerpt() ?: wp_trim_words(get_post_field('post_content', get_the_ID()), 75, ''),
...
// Register custom REST API endpoint
add_action('rest_api_init', function () {
    register_rest_route('custom/v1', '/insites', array(
        'methods' => 'GET',
        'callback' => 'get_insites_posts',
        'permission_callback' => '__return_true',
        'args' => array(
            'infoscreen' => array(
                'validate_callback' => function ($param, $request, $key) {
...
// Callback function to handle the custom endpoint
function get_insites_posts($request) {
    // Get the value of infoscreen, site, and per_page parameters from the request
    $infoscreen = $request->get_param('infoscreen');
    $site = $request->get_param('site');
    $per_page = $request->get_param('per_page');
    ...
    if ($query->have_posts()) {
        ...
        while ($query->have_posts()) {
            ...
            $post_data = array(
                ...
                'featured_image_src'=> get_the_post_thumbnail_url(get_the_ID(), 'full'), // Featured image source URL
                'infoscreen'        => get_field('infoscreen')
            );
